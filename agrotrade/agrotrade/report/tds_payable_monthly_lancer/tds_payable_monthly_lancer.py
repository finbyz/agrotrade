...
def get_result(filters, tds_docs, tds_accounts, tax_category_map):
	...
	total = get_purchase_total(filters)
	...
	for name, details in gle_map.items():
		...
		if tds_deducted:
			...
			for d in total:
				if row.get('ref_no') == d.name:
					row.update({'net_total':d.total})
			jv_data = journal_entry_map()
			for d in jv_data:
				if (not row.get('section_code')) or (row.get('section_code') == ""):
					if row.get('ref_no') == d.name:
						row.update({'section_code':d.tds_section_code})
				if (not row.get('pan')) or (row.get('pan') == ""):
					if row.get('ref_no') == d.name:
						row.update({'pan':d.pan})
				
			out.append(row)

	return out

def journal_entry_map():
	jv_data = frappe.db.sql("""
					SELECT 
						jv.pan , jv.tds_section_code , jv.name
					FROM
						`tabJournal Entry` as jv
					Where
						tds_section_code IS NOT NULL
	""",as_dict=1)
	return jv_data
...
def get_purchase_total(filters):
	total = frappe.db.sql("""
		...
		from
			`tabPurchase Invoice`
		Where
			tax_withholding_category IS NOT NULL 
