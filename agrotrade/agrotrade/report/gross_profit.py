...
def execute(filters=None):
	...
	group_wise_columns = frappe._dict({
		"invoice": ["invoice_or_item", "po_no", "customer", "customer_group", "posting_date","item_code", "item_name","item_group", "description",
...
def get_columns(group_wise_columns, filters):
	...
	column_map = frappe._dict({
		...
		"invoice_or_item": {"label": _('Sales Invoice'), "fieldtype": "Link", "options": "Sales Invoice", "width": 140},
		...
		"po_no": {"label": _('PO No'), "fieldname": "po_no", "fieldtype": "Data", "width": 120},
...
class GrossProfitGenerator(object):
	...
	def get_returned_invoice_items(self):
		returned_invoices = frappe.db.sql("""
			select
				si.name, si_item.item_code, si_item.stock_qty as qty, si_item.base_net_amount as base_amount, si.return_against, si.po_no
			from
	...
	def load_invoice_items(self):
		...
		self.si_list = frappe.db.sql("""
			select
				...
				`tabSales Invoice Item`.cost_center,`tabSales Invoice`.po_no
				{sales_person_cols}
			from
