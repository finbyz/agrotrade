...
def get_purchase_order_data():
	...
	data = frappe.db.sql("""
		select
			sum(po.qty) as po_qty, po.sales_order, po.sales_order_item, 
			pop.schedule_date, pop.supplier , po.name as po_name  , po.qty as qty
		from
			`tabPurchase Order Item` as po
			JOIN `tabPurchase Order` as pop on pop.name = po.parent 
		where
			po.docstatus = 1 and po.sales_order_item is not null and po.sales_order is not null
		group by po.sales_order, po.sales_order_item 
		""", as_dict=1)
	
	for po in data:
		if po.sales_order_item:
			po_map.setdefault(po.sales_order_item,po)
...
def get_columns_details():
	columns = [
		...
		{ "label": _("Pending Po Qty"),"fieldname": "panding_qty","fieldtype": "Float","width": 70},
