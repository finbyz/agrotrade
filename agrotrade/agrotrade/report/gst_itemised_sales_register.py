...
def execute(filters=None):
    return _execute(filters, additional_table_columns=[
        ...
        dict(fieldtype='Data', label='Reverse Charge', fieldname="is_reverse_charge", width=120),
...
def get_conditions(filters):
    ...
    if filters.get("is_return"):
        conditions += """and `tabSales Invoice`.is_return = 1 """

    if filters.get("item_group"):
        conditions +=  """and ifnull(`tabSales Invoice Item`.item_group, '') = %(item_group)s"""
    if filters.get("gst_category"): 
        conditions +=  f"""and `tabSales Invoice`.gst_category = '{filters.gst_category}'"""
