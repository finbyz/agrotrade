...
def get_result(filters, account_details):
	...
	#finbyz Changes
	final_result =[]
	if filters.get('fx_account'):
		for row in result:
			if not row.get('voucher_no'):
				final_result.append(row)
			else:
				currency =frappe.db.get_list("GL Entry","account_currency",filters={"voucher_no":row.get('voucher_no')},pluck="account_currency")
				u_currency = set(currency)
				if u_currency != {'INR'}:
					final_result.append(row) 
		return final_result
	else:		
